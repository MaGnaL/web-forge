...
When a console group is created with `cnsl.group` or `cnsl.groupCollapsed` a grouped cnsl instance is returned, which buffers all actions until `grpdCnsl.groupEnd` is called.
...
Here is this sample with the "usual" implementation:
...
* categorized logs to toggle dedicated topics (e.g. show only api related logs, show all logs except some)
